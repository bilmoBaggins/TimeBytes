...
import { enrollEmployeeFace } from "../cloud/faceRecognition";
...
export default function FaceEnrollmentModal({ employee, onClose, onEnrolled }: Props) {
  ...
  async function captureFace() {
    if (!cameraRef.current || !employee || busy) return;
    setBusy(true);
    setMessage("");
    try {
      ...
      const faceId = await enrollEmployeeFace(employee.id, photo.base64);
      await onEnrolled(faceId);
      closeModal();
    } catch (error: any) {
      setMessage(error.message || "Could not enroll this face.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Modal visible={employee !== null} animationType="slide" onRequestClose={closeModal}>
      <View style={styles.container}>
        <Text style={styles.title}>Enroll {employee?.name}</Text>
        <Text style={styles.subtitle}>Look at the camera in good light, then tap capture.</Text>
        {!permission ? (
          <ActivityIndicator color="#F28C00" />
        ) : !permission.granted ? (
          <View style={styles.permissionBox}>
            <Text style={styles.message}>Camera access is needed to enroll a face.</Text>
          ...
          <>
            ...
            <Pressable style={styles.primaryButton} onPress={captureFace} disabled={busy}>
              {busy ? <ActivityIndicator color="white" /> : <Text style={styles.buttonText}>Capture Face</Text>}
            </Pressable>
          </>
        )}
